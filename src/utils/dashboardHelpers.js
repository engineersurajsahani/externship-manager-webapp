...
export const dashboardHelpers = {
  ...
  generateSampleData: (userRole, userEmail) => {
    const sampleProjects = [
      { id: 1, name: 'E-commerce Platform', status: 'active', startDate: '2024-01-10', interns: ['John Doe'] },
      { id: 2, name: 'Mobile App Development', status: 'active', startDate: '2024-01-15', interns: ['Jane Smith'] },
      { id: 3, name: 'Data Analytics Dashboard', status: 'completed', startDate: '2023-12-01', interns: ['Bob Wilson'] }
    ];

    const sampleUsers = [
      { id: 1, name: 'John Doe', email: 'john@example.com', role: 'intern', teamLeader: 'Jane Manager' },
      { id: 2, name: 'Jane Smith', email: 'jane@example.com', role: 'intern', teamLeader: 'Jane Manager' },
      { id: 3, name: 'Bob Wilson', email: 'bob@example.com', role: 'intern', teamLeader: 'Jane Manager' },
        { id: 4, name: 'Alice Johnson', email: 'alice@example.com', role: 'tl', title: 'Team Leader' },
        { id: 5, name: 'Mike Davis', email: 'mike@example.com', role: 'pm', title: 'Project Manager' }
    ];
    ...
    const sampleDailyUpdates = [
      {
        ...
        planTomorrow: 'Work on dashboard components',
      ...
      },
      {
        ...
        planTomorrow: 'Continue with authentication',
  ...
  calculateRealTimeStats: (userRole, userEmail, updates, projects, users) => {
    ...
    switch (userRole) {
      case 'intern':
        ...
        const myProjects = projects.filter(p => p.interns?.includes(userEmail?.split('@')[0]));
        return {
          ...
          todayStatus: myUpdates.some(u => u.date === today) ? 'Submitted' : 'Pending'
        };
        
      case 'tl':
        const teamMembers = users.filter(u => u.teamLeader === userEmail?.split('@')[0]);
        const teamUpdates = updates.filter(u => 
          teamMembers.some(member => member.email === u.userEmail)
        );
        return {
          teamSize: teamMembers.length,
          projectsManaged: projects.filter(p => p.teamLeaders?.includes(userEmail?.split('@')[0])).length,
          teamAttendance: Math.round((teamUpdates.length / Math.max(teamMembers.length * 7, 1)) * 100),
          pendingReviews: teamUpdates.filter(u => u.status === 'pending').length
        };
        
      case 'pm':
        const managedProjects = projects.filter(p => p.lead === userEmail?.split('@')[0]);
        return {
          totalTeamMembers: users.filter(u => u.role === 'intern').length,
          activeProjects: managedProjects.filter(p => p.status === 'active').length,
          overallAttendance: Math.round((updates.length / Math.max(users.length * 7, 1)) * 100),
      ...
      default:
        return {
          ...
          attendanceRate: Math.round((updates.length / Math.max(users.length * 7, 1)) * 100) || 87
        };
    }
  },
  ...
  calculateAttendanceRate: (updates) => {
    ...
    return pastWorkingDays > 0 ? Math.round((currentMonthUpdates.length / pastWorkingDays) * 100) : 0;
  },
  ...
  getRecentActivity: (updates, projects) => {
    const recent = [...updates]
      ...
      .map(update => ({
        ...
        description: update.workDone?.substring(0, 50) + '...' || 'Update submitted',
  ...
  generateNotifications: (userRole, userEmail, updates, projects) => {
    ...
    if (userRole === 'intern') {
      const todayUpdate = updates.find(u => u.date === today && u.userEmail === userEmail);
      if (!todayUpdate) {
        notifications.push({
          id: 'update-reminder',
          type: 'warning',
          title: 'Daily Update Pending',
          message: 'Keep your streak going! Submit your daily update for today.',
          action: 'submit-update'
        });
      }
      
      // Check streak warning
      const streak = dashboardHelpers.calculateStreak(updates.filter(u => u.userEmail === userEmail));
      if (streak >= 5) {
        notifications.push({
  ...
  ATTENDANCE_DEADLINE_HOUR: 18, // 6 PM
  ATTENDANCE_DEADLINE_MINUTE: 0,
  
  // Time remaining helper (enhanced for attendance deadline)
  getTimeRemainingToday: () => {
    const now = new Date();
    const deadline = new Date();
    deadline.setHours(dashboardHelpers.ATTENDANCE_DEADLINE_HOUR, dashboardHelpers.ATTENDANCE_DEADLINE_MINUTE, 0, 0);
    
    // If current time is past deadline, return 0 time remaining
    if (now > deadline) {
      return '0h 0m';
    }
  ...
  hasAttendanceDeadlinePassed: () => {
    ...
    todayDeadline.setHours(dashboardHelpers.ATTENDANCE_DEADLINE_HOUR, dashboardHelpers.ATTENDANCE_DEADLINE_MINUTE, 0, 0);
